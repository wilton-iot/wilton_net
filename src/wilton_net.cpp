...
const std::string logger = std::string("wilton.Socket");

wilton::net::wilton_socket create_socket(const std::string& ip_addr, uint16_t port,
        const std::string& protocol, const std::string& role, std::chrono::milliseconds timeout) {
    if ("TCP" == protocol) {
        if ("server" == role) {
            return wilton::net::tcp_server_socket(ip_addr, port, timeout);
        } else if("client" == role) {
            return wilton::net::tcp_client_socket(ip_addr, port, timeout);
        } else {
        ...
        if ("server" == role) {
            return wilton::net::udp_server_socket(ip_addr, port, timeout);
        } else if("client" == role) {
            return wilton::net::udp_client_socket(ip_addr, port, timeout);
        } else {
            throw wilton::support::exception(TRACEMSG("Invalid 'role' parameter" +
                    " specified: [" + role + "], must be one of: [server, client]"));
        }
...
char* wilton_net_Socket_open(wilton_Socket** socket_out,
        const char* ip_addr, int ip_addr_len, int port,
    ...
    if (!sl::support::is_uint16_positive(port)) return wilton::support::alloc_copy(TRACEMSG(
            "Invalid 'port' parameter specified: [" + sl::support::to_string(port) + "]"));
    ...
    try {
        ...
        auto socket = create_socket(ip_addr_str, static_cast<uint16_t>(port), protocol_str, role_str, timeout);
...
char* wilton_net_Socket_write(wilton_Socket* socket, const char* data, int data_len,
            int timeout_millis, int* bytes_written) /* noexcept */ {
    if (nullptr == socket) return wilton::support::alloc_copy(TRACEMSG("Null 'socket' parameter specified"));
    ...
    if (!sl::support::is_uint32(timeout_millis)) return wilton::support::alloc_copy(TRACEMSG(
            "Invalid 'timeout_millis' parameter specified: [" + sl::support::to_string(timeout_millis) + "]"));
    if (nullptr == bytes_written) return wilton::support::alloc_copy(TRACEMSG("Null 'bytes_written' parameter specified"));
    try {
        wilton::support::log_debug(logger, std::string("Writing data to socket,") +
                ...
                " timeout: [" + sl::support::to_string(timeout_millis) + "] ...");
        auto written = socket->impl().write({data, data_len}, std::chrono::milliseconds(timeout_millis));
        wilton::support::log_debug(logger, "Write operation complete, bytes written: [" + sl::support::to_string(written) + "]");
        *bytes_written = static_cast<int> (written);
        return nullptr;
    } catch (const std::exception& e) {
        return wilton::support::alloc_copy(TRACEMSG(e.what() + "\nException raised"));
    }
...
char* wilton_net_Socket_read_some(wilton_Socket* socket, int timeout_millis,
    ...
    try {
        ...
        auto buf = span.size() > 0 ? 
            wilton::support::make_const_span_buffer(span) :
            wilton::support::make_empty_buffer();
        *data_out = buf.data();
...
char* wilton_net_Socket_read(wilton_Socket* socket, int bytes_to_read, int timeout_millis,
    ...
    try {
        ...
        auto read = socket->impl().read(buf, std::chrono::milliseconds(timeout_millis));
        wilton::support::log_debug(logger, std::string("Read operation complete,") +
                " bytes read: [" + sl::support::to_string(read) + "]");
        if (buf.size() == read) { // full read
            *data_out = buf.data();
            *data_len_out = buf.size_int();
            defer_killswitch = true;
        } else if (read > 0) { // partial read
            auto res = wilton::support::alloc_span(read);
            std::memcpy(res.data(), buf.data(), read);
            *data_out = res.data();
            *data_len_out = res.size_int();
        } else { // empty read
            auto res = wilton::support::make_empty_buffer();
            *data_out = res.data();
            *data_len_out = 0;
        }
        return nullptr;
...
char* wilton_net_wait_for_tcp_connection(const char* ip_addr, int ip_addr_len, 
        int port, int timeout_millis) /* noexcept */ {
    if (nullptr == ip_addr) return wilton::support::alloc_copy(TRACEMSG("Null 'ip_addr' parameter specified"));
    ...
    try {
        ...
        std::string err = wilton::net::tcp_operations::wait_for_connection(
                ip_addr_str, static_cast<uint16_t>(port), std::chrono::milliseconds(timeout_millis));
